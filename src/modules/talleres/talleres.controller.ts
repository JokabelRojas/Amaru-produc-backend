...
export class TalleresController {
  ...
  async findAll(): Promise<Taller[]> {
    ...
    @Get('profesor/:id')
  @ApiOperation({
    summary: 'Obtener talleres dictados por un profesor',
    description: 'Retorna la lista de talleres que pertenecen al profesor indicado.',
  })
  findByProfesor(@Param('id') id: string) {
    return this.talleresService.findByProfesor(id);
  }
